...
# ===== CONFIG =====
CG1050_TRAIN_DIR = "dataset_cg1050/train"  # Stage 1 dataset
TXN_TRAIN_DIR = "dataset/train"            # Stage 2 dataset
VAL_DIR = "dataset/val"                     # Validation set for Stage 2
...
# ===== TRAIN FUNCTION =====
def train_stage(model, train_loader, val_loader=None, epochs=3, lr=1e-3):
...
if __name__ == "__main__":
    ...
    model = build_model()
    model = train_stage(model, cg_loader, epochs=3, lr=1e-3)
    ...
    model = train_stage(model, txn_loader, val_loader, epochs=5, lr=5e-4)
